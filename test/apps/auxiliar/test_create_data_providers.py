...
def test_main_00(db_session: Session) -> None:
    """
    ...
    """
    assert db_session.scalar(select(func.count(DataProvider.data_provider_name))) == 0
    main(db_session)
    assert db_session.scalar(select(func.count(DataProvider.data_provider_name))) == 2
